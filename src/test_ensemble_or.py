...
def avrg_mask(full_size_mask, stride):
    ...
    """
    _, height, width, _ = full_size_mask.shape
...
def test(config):
    ...
    for path in tqdm.tqdm(os.listdir(config["test"]["test_path"])):
        ...
        with torch.no_grad():
            ...
            full_size_mask = np.zeros((len(model_ensemble_name), *full_size_image.shape[:2], 2))
            for start_height, start_width, image in generate_crops(full_size_image, config["test"]["stride"]):
                ...
                for i in range(len(model_ensemble_name)):
                    full_size_mask[i, start_width: start_width + WINDOW_SIZE, start_height:start_height + WINDOW_SIZE, :] += prediction_mask[i]
